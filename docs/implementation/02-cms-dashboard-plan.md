...
- [ ] Create repo `golden-cms-dashboard`
- [ ] Scaffold Next.js App Router + TypeScript + Tailwind
- [ ] Add Cairo font, global `dir="rtl"` on `<html lang="ar">`
- [ ] Create `lib/copy/ar.ts` with sidebar + common actions from glossary
- [ ] Add `.env.example`, README (requires API URL)
- [ ] Placeholder `(dashboard)/layout.tsx` with Sidebar mock links
- [ ] Placeholder `/overview` with **ماذا تريد أن تعدّل؟**
